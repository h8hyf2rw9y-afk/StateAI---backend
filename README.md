...
🚧 **Core CRM (Contacts/Properties/Buyer Requirements/Property Interests/Activities/Features) + deterministic Buyer Matching + the sales Pipeline (Opportunities) + three read-only AI agents (Lead Intelligence, Follow-up, Pipeline) behind an AI Gateway + a production-readiness hardening pass + self-service Organization/User onboarding** (Audit Log, AI execution persistence, Tasks, Appointments, a prepared-but-stubbed Calendar Integration layer, a Notification foundation, minimal role authorization, global structured error handling, and — as of the most recent phase — `POST /me/organization` closing the last "must run a manual SQL `INSERT`" gap for a brand-new signup) — live on the real Supabase Postgres database, seeded with realistic demo data, and covered by 337 passing tests. The real frontend is wired to essentially all of this now too (Leads, Properties, Pipeline, Tasks, Appointments, Buyer Matching, all three AI agents, a real Dashboard, and now real sign-up/sign-in onboarding — see the frontend's own README). Still no Transactions, Commissions, Documents, real OAuth calendar sync, notification delivery (email/push/SMS) or a Notifications frontend, team invites into an *existing* organization, Sales Copilot, or any autonomous AI action (every agent remains strictly read-only/advisory) — see [What's next](#whats-next) for exactly what's real versus prepared-but-not-implemented.
...
- Wiring the frontend to Notifications specifically (Lead Intelligence, Follow-up, Pipeline, Tasks, Appointments, and Opportunities are all real now — see the frontend's own README) — Notifications is the one entity left with no frontend surface.

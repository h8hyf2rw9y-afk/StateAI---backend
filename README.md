...
**Execution safety:** every POST accepts an optional `Idempotency-Key` header. Repeating the same request with the same key reuses its execution/result instead of charging for another provider call; reusing that key for a different user, contact, or agent is rejected. A partial unique database index allows only one `queued`/`running` execution for the same organization + contact + agent while preserving the complete succeeded/failed history. If a duplicate arrives while the first run is active, the API returns `202` with the existing `execution_id`, and the frontend follows that execution instead of starting another. A completed run also starts a configurable cooldown, returned as structured `429` metadata plus `Retry-After`; durable per-user, per-organization, and per-contact/agent limits provide an additional cost boundary across server processes. Configure these with `AI_AGENT_COOLDOWN_SECONDS`, `AI_USER_RATE_LIMIT_PER_MINUTE`, `AI_ORGANIZATION_RATE_LIMIT_PER_MINUTE`, and `AI_CONTACT_AGENT_RATE_LIMIT_PER_HOUR`.
...
- **Failure** (`LLMTimeoutError`/`LLMInvalidOutputError`/`LLMProviderError`) updates the reserved row to `status="failed"`, records `completed_at`, and stores `output={"error": "<exception class>", "message": "..."}` — timed by the route itself since no `GatewayExecution` exists to read a duration from. The failed row no longer occupies the single-flight slot, so a later retry can proceed.
